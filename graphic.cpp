...
public:
    ...
    void pre_iter(int i, int * a) {
        ...
        setcolor(WHITE);
        h = i * 2;
        bar(left = h, top = BARMINHEIGHT + a[i], right = h + BARWIDTH, bottom = 500);
    }
    ...
    void post_iter(int * a) {
        ...
        for (int i = 1; i <= n; i++) {
          h = i * 2;
          bar(left = h, top = BARMINHEIGHT + a[i], right = h + 2, bottom = 500);
      }
  }

};

class Alogrithms: public Graphic {
